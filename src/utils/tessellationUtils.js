 ...
 * Satisfies the mathematical constraints from the Python tessellation_test project:
      ...
      if (shouldSubdivide) {
        ...
        if (subdivisionType === 0) {
          ...
          subdivisions.forEach(points => {
            const polygonPoints = points.map(point => ({ x: point.x, y: point.y }));
...
export const validateTessellation = (tiles, width, height) => {
  ...
  tiles.forEach((tile, index) => {
    ...
    const outsidePoints = tile.points.filter(point => 
...
/**
 * Creates a target area function based on distance from center.
 * This is adapted from the tessellation_test Python code.
 * @param {number} width - Width of the canvas
 * @param {number} height - Height of the canvas
 * @param {number} tileCount - Total number of tiles
 * @param {string} profile - 'larger-center', 'smaller-center', or 'uniform'
 * @returns {Function} - Function that takes a point and returns target area
 */
export const createTargetAreaFunction = (width, height, tileCount, profile = 'uniform') => {
  const center = [width / 2, height / 2];
  const maxDist = Math.sqrt((width/2) ** 2 + (height/2) ** 2);
  const baseArea = (width * height) / tileCount;
  const factor = 0.8; // Strength of gradient
  ...
  if (profile === 'uniform') {
    return () => baseArea;
  } else if (profile === 'larger-center') {
    return (p) => baseArea * (1 + factor * (1 - (calculateDistance(p, center) / maxDist)));
  } else if (profile === 'smaller-center') {
    return (p) => baseArea * (1 + factor * (calculateDistance(p, center) / maxDist));
  }
